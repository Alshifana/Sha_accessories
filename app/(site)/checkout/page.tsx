...
export default function CheckoutPage() {
  ...
  const finishOrder = (orderId: string, isGuestCheckout: boolean) => {
  ...
  const placeOrder = async () => {
    ...
    if (!user) {
      finishOrder(crypto.randomUUID().slice(0, 8).toUpperCase(), true);
      return;
    }
    ...
    finishOrder(orderRow.id.slice(0, 8).toUpperCase(), false);
  };
